...
(function () {
    ...
    SHOCK.get_all_nodes = function (ret,params) {
	var url = SHOCK.url+'/node'+(params ? params : "");
	var promise = jQuery.Deferred();
        jQuery.ajax(url, { 
	    success: function(data) {
		var retval = null;
    ...
    SHOCK.create_node_from_JSON = function (inputData, ret) {
	var url = SHOCK.url+'/node';
	var promise = jQuery.Deferred();

	var aFileParts = [ JSON.stringify(inputData) ];
	...
	if (input != null) {
		...
		var frOnload = function(e) {
		    ...
		    fd.append(SHOCK.currentChunk+1, oMyBlob, file.name);
		    jQuery.ajax(url, {
			xhr: function() {
	...
	if (node != null) {
	    var aFileParts = [ JSON.stringify((attr ? attr : {})) ];
	    var oMyBlob = new Blob(aFileParts, { "type" : "text\/json" });
	    var fd = new FormData();
	    fd.append('attributes', oMyBlob);
